...
export default function Copygen({
  ...
  useEffect(() => {
    ...
    if (paragraphwriter != null && paragraphwriter.length > 0) {
      setTopic(paragraphwriter[0].topic);
      setTargetkeywords(paragraphwriter[0].targetkeywords);
      setToneofvoice(paragraphwriter[0].toneofvoice);
    }
  }, [paragraphwriter, toolgen]);
  ...
  const handletitle = async (event: any) => {
    ...
    const updatetitle = await fetch(
      "/api/getcopy/paragraphwriter/updatetitle",
      {
        method: "POST",
        headers: {
  ...
  async function handleSubmit(event: any) {
    ...
    try {
      ...
      const targetkeywords = event.target.targetkeywords.value;
      const toneofvoice = event.target.toneofvoice.value;

      const response = await fetch("/api/getcopy/paragraphwriter", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (data.status == "success") {
        ...
        if (data.act == "update") {
          router.push("/copygen/paragraphwriter/" + proid);
        } else {
          router.push("/copygen/paragraphwriter/" + data.proid);
        }
        return;
      }
    } catch (error) {
  ...
  const saveCopyToFav = async (id: any, isSaved: any) => {
    const response = await fetch("/api/getcopy/paragraphwriter/savecopytofav", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const saveCopy = async (id: any) => {
    ...
    const proid = router.query.toolgenId;
    if (edit) {
      const response = await fetch("/api/getcopy/paragraphwriter/editcopy", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const deleteCopy = async (id: any) => {
    ...
    const response = await fetch("/api/getcopy/paragraphwriter/deletecopy", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
export async function getServerSideProps(context: any) {
  ...
  if (proid) {
    ...
    const paragraphwriter = await prisma.paragraphwriter.findMany({
      select: {
        id: true,
        topic: true,
        targetkeywords: true,
