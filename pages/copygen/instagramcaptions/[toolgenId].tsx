...
export default function Copygen({
  ...
  useEffect(() => {
    ...
    if (instagramcaptions != null && instagramcaptions.length > 0) {
      setDescription(instagramcaptions[0].description);
      setToneofvoice(instagramcaptions[0].toneofvoice);
    }
  }, [instagramcaptions, toolgen]);
  ...
  const handletitle = async (event: any) => {
    ...
    const updatetitle = await fetch(
      "/api/getcopy/instagramcaptions/updatetitle",
      {
        method: "POST",
        headers: {
  ...
  async function handleSubmit(event: any) {
    ...
    try {
      ...
      const response = await fetch("/api/getcopy/instagramcaptions", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (data.status == "success") {
        ...
        if (data.act == "update") {
          router.push("/copygen/instagramcaptions/" + proid);
        } else {
          router.push("/copygen/instagramcaptions/" + data.proid);
        }
        return;
      }
    } catch (error) {
  ...
  const saveCopyToFav = async (id: any, isSaved: any) => {
    const response = await fetch(
      "/api/getcopy/instagramcaptions/savecopytofav",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const saveCopy = async (id: any) => {
    ...
    const proid = router.query.toolgenId;
    if (edit) {
      const response = await fetch("/api/getcopy/instagramcaptions/editcopy", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const deleteCopy = async (id: any) => {
    ...
    const response = await fetch("/api/getcopy/instagramcaptions/deletecopy", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
export async function getServerSideProps(context: any) {
  ...
  if (proid) {
    ...
    const instagramcaptions = await prisma.instagramcaptions.findMany({
      select: {
        id: true,
        description: true,
        toneofvoice: true,
