...
export default function Copygen({ formattedCopy, emailcopy, toolgen }: any) {
  ...
  useEffect(() => {
    ...
    if (emailcopy != null && emailcopy.length > 0) {
      setTo(emailcopy[0].to);
      setCompanyname(emailcopy[0].companyname);
      setContexts(emailcopy[0].contexts);
      setPurpose(emailcopy[0].purpose);
      setToneofvoice(emailcopy[0].toneofvoice);
    }
  }, [emailcopy, toolgen]);
  ...
  const handletitle = async (event: any) => {
    ...
    const updatetitle = await fetch("/api/getcopy/emailcopy/updatetitle", {
      method: "POST",
      headers: {
  ...
  async function handleSubmit(event: any) {
    ...
    try {
      ...
      const contexts = event.target.contexts.value;
      const purpose = event.target.purpose.value;
      const toneofvoice = event.target.toneofvoice.value;

      const response = await fetch("/api/getcopy/emailcopy", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (data.status == "success") {
        ...
        if (data.act == "update") {
          router.push("/copygen/emailcopy/" + proid);
        } else {
          router.push("/copygen/emailcopy/" + data.proid);
        }
        return;
      }
    } catch (error) {
  ...
  const saveCopyToFav = async (id: any, isSaved: any) => {
    const response = await fetch("/api/getcopy/emailcopy/savecopytofav", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const saveCopy = async (id: any) => {
    ...
    const proid = router.query.toolgenId;
    if (edit) {
      const response = await fetch("/api/getcopy/emailcopy/editcopy", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const deleteCopy = async (id: any) => {
    ...
    const response = await fetch("/api/getcopy/emailcopy/deletecopy", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
export async function getServerSideProps(context: any) {
  ...
  if (proid) {
    ...
    const emailcopy = await prisma.emailcopy.findMany({
      select: {
        id: true,
        to: true,
        companyname: true,
