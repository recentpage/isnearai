...
const openai = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const toolId = "cld4swpc30000v694xjzc78eh";
  const { proid, toneofvoice, description } = req.body;
  ...
  const prompt = `Generate a instagram caption for a product with the following attributes: description = '${description}'.use Tone of voice = '${toneofvoice}'. each variation has 200 chars long. new caption starts with new line.genrate 3 variations of the product description.`;
  // console.log(prompt);

  try {
    //check if the user has enough credits to generate the product description if not return error
    const checkCredits = await getCredits(userId, "instagramcaptions");
    if (!checkCredits) {
      res
        .status(401)
      ...
      if (response.status === 200) {
        ...
        if (proid === "blank") {
          ...
          if (toolgen.id && description) {
            ...
            const addProductdescription = await prisma.instagramcaptions.create(
              {
                data: {
          ...
          const addProductdescription = await prisma.instagramcaptions.update({
            where: {
              toolgenId: proid,
            },
            data: {
